...
{
    [SerializeField] private int widthX = 30;
    [SerializeField] private int widthZ = 30;
    [SerializeField] private int heightY = 30;
    ...
    {
        heights = new float[widthX + 1, heightY + 1, widthZ + 1];
            ...
            for (int y = 0; y < heightY + 1; y++)
            {
                for (int z = 0; z < widthZ + 1; z++)
                {
                    if (use3DNoise)
                    {
                        if (isMove)
                        {
                            float currentHeight = PerlinNoise3D((float)x / widthX * noiseScale,
                                (float)y / heightY * noiseScale, (float)(z + offset) / widthX * noiseScale);
                            heights[x, y, z] = currentHeight;
                        }
                        else
                        {
                            float currentHeight = PerlinNoise3D((float)x / widthX * noiseScale,
                                (float)y / heightY * noiseScale, (float)z / widthX * noiseScale);
                            heights[x, y, z] = currentHeight;
                        }
                    }
                    else
                    {
                        float currentHeight = heightY * Mathf.PerlinNoise(x * noiseScale, z * noiseScale);
                        float distToSufrace;
        ...
        for (int x = 0; x < widthX; x++)
        {
            for (int y = 0; y < heightY; y++)
            {
                for (int z = 0; z < widthZ; z++)
                {
                    ...
                    {
                        Vector3Int
                            corner = new Vector3Int(x, y, z) +
                                     MarchingTable.Corners[i]; //定位每个小立方体的8个顶点在标量场中的绝对位置（这是一个三维int向量：Vector3Int）
                        cubeCorners[i] =
                            heights
                            [corner.x, corner.y,
                                corner.z]; //根据小立方体的8个顶点的绝对位置的xyz坐标得到对应的柏林噪声的数值（因为corner的xyz三个分量和heights的三个分量都是分别由width，height，width来决定的，所以可以准确定位到）
                    }

                    MarchCube(new Vector3Int(x, y, z),
                        cubeCorners); //在第三层的单次循环中，cubeCorners储存了单个march cube中的8个顶点的噪声值。向MarchCube中传入单个立方体的位置和8个顶点的噪声数据数组
                }
            }
        }
    }
    ...
    private void MarchCube(Vector3Int position, float[] cubeCorners)
    {
        int configIndex =
            GetConfigIndex(
                cubeCorners); //将单个立方体的顶点噪声数据传入GetConfigIndex函数中，与heightThreshold比较 来生成配置索引，这是一个8位的数值，对应立方体的八个顶点。表示单个立方体的8个顶点是否在需要构建的形状的内部（如在形状外还是形状内）

        if (configIndex == 0 ||
            configIndex ==
            255) //如果 configIndex是0000 0000或1111 1111，即立方体的所有点都在需要构建的图形内部或者外部，因为立方体没有边和图形相交，所以不需要参与图形重构（不需要生成新的顶点和三角形）
        {
            return;
        }
        ...
        for (int t = 0; t < 5; t++) //一个立方体中三角形的最大数量为5
        {
            ...
            {
                int triTableValue =
                    MarchingTable.Triangles[configIndex, edgeIndex]; //得到MarchingTable.Triangles[,]这个二维数组的边索引
    ...
    private Vector3 InterpolateEdgePosition(float threshold, Vector3 vertex1, float value1, Vector3 vertex2,
        float value2)
    {
    ...
    private int GetConfigIndex(float[] cubeCorners)
    {
        ...
        for (int i = 0; i < 8; i++) //循环遍历立方体的8个顶点
        {
             ...
             */
            if (cubeCorners[i] >
                heightThreshold) //判断顶点是否在等值面的内部，如果heightThreshold为1，（因为柏林噪声的最大值就为1）所以没有点会大于heightThreshold，即所有点都不在形状内部，也就是说不会形成形状
            {
                //用二进制位标记顶点状态，生成配置索引。
                configIndex |= 1 << i;
            }
